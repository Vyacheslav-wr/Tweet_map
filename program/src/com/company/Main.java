...
public class Main
{
    ...
    // Слова
    public static Dictionary WordsRead()
    {
        ...
        try
        {
            File file = new File("sentiments.csv");
            //создаем объект FileReader для объекта File
            FileReader fr = new FileReader(file);
            //создаем BufferedReader с существующего FileReader для построчного считывания
            BufferedReader reader = new BufferedReader(fr);
    ...
    public static ArrayList<State> StatesRead ()
    {
        ...
        while(e.hasMoreElements()) {
            ...
            if (oldPolygons.size() > 1) // если полигонов много
            {
                for (int i = 0; i < oldPolygons.size(); i++) {
                    ...
                    /*
                    newPolygon.moveTo(newPoints.get(0).x, newPoints.get(0).y);
                    for(int q = 1; q < newPoints.size(); ++q)
                    {
    ...
    public static Dictionary GetOpinion (ArrayList<Tweet> tweets, Dictionary d, Dictionary f)
    {
        double opinion = 0;
        Dictionary dic = new Hashtable();

        Enumeration<State> en = f.keys();
        ...
        while(en.hasMoreElements())
        {
            ...
            for (Tweet t:  (ArrayList<Tweet>)f.get(s))
            {
    ...
    public static void main(String[] args)
    {
        ...
        // Словарь слов и весов
        Dictionary k = WordsRead();

        // Список штатов
        ArrayList<State> states = StatesRead();

        Dictionary f = StatesAndTweets(tweets, states);

        Dictionary o = GetOpinion(tweets, k, f);
